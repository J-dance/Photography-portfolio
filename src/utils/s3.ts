...
const listBucketItems = async (bucketName: string) => {
...
const createImgUrl = (key: string) => {
  return `https://${import.meta.env.DB_BUCKET}.s3.${
    import.meta.env.DB_REGION
  }.amazonaws.com/${key}`;
};
...
export const getS3Data = async () => {
  const bucketItems = await listBucketItems(import.meta.env.DB_BUCKET);
